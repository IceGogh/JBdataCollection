...
if( $power == 0 ){
    ...
    //  全局查看
    $result = mysqli_query( $con,"SELECT id FROM user ".@$f_team.@$f_customer.@$f_status.@$f_from);
...
if( $power == 0 ){
    ...
    //  全局查看
    $sql = "select * from user ".@$f_team.@$f_customer.@$f_status.@$f_from." order by id desc limit $startPage, $endPage";
    echo $sql;
}else if( $power == 1 ){
